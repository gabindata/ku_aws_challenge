...
> [!WARNING]
> **구형 설계 문서입니다.** 이 문서의 협상 판정 방식(`trust`, `agreementGap`,
> `dealClosed`, 게이지)은 기획 공통규칙으로 대체되었습니다. 현재 기준은
> 합의 이벤트(`confirm`/`revoke`/`keep`/`clarify`) + 10분 타이머이며,
> 실제 계약은 `shared/types/negotiationTypes.ts`가 단일 소스입니다.
>
> 3장(씬 구조), 6장 지표 목록, 8장 폴더 구조는 아직 유효합니다.
