...
export interface MergeResult {
  ...
  newlyMetKeys: string[];
  /**
   * 이미 met이던 키의 합의 내용이 바뀐 것. NPC가 바뀐 값을 복창하고 메모를 교체한다.
   * "오늘 대출 신청 대신 내일 포털에서 납부하겠습니다" 같은 대안 교체가 여기 해당한다.
   */
  updatedKeys: string[];
...
export function applyJudgements(
  ...
  for (const [key, judgement] of Object.entries(llm.judgements)) {
    ...
    if (!wasMet) {
      result.newlyMetKeys.push(key);
    } else if (summaryChanged) {
      // 같은 키의 내용이 바뀐 경우. 메모를 교체하고 NPC가 바뀐 값을 복창한다.
      result.updatedKeys.push(key);
    }
    // 이미 met이고 내용도 같으면 아무것도 보고하지 않는다. 같은 값을 반복 복창하지 않는다.
  }
...
export function resolveOutcome(
  ...
  // 3. 치명적 행동
  //    fatalRecovery: true면 종료하지 않고 직전 상태로 되돌린다. 세션은 이어진다.
  if (input.fatal) {
    return stage.fatalRecovery === true
      ? { outcome: 'reverted', endReason: null }
      : { outcome: 'failure', endReason: 'fatal' };
  }
...
/**
 * 종료 화면의 표정. LLM이 정하지 않는다.
 * 종료 시점에는 LLM을 호출하지 않으므로 서버가 스테이지 정의의 고정값을 쓴다.
 */
...
/**
 * 종료 응답에 실을 결과. 모든 문구가 스테이지 정의에서 온다.
 *
 * failureText는 endReason과 무관하게 모든 실패 화면 상단에 온다.
 * epilogue는 성공일 때만 채운다 — 실패·수정 재요청 응답에는 넣지 않는다.
 */
export function buildResult(
  ...
  return {
    failureText: failure ? stage.failureText ?? null : null,
    outcomeText: success
      ? stage.successText
      : endReason === 'limit'
        ? stage.limitText
        : null,
    fixedTerms: success ? stage.fixedTerms : [],
    successState: success ? stage.successState : null,
    epilogue: success ? stage.successEpilogue ?? null : null,
    onClose: stage.onFailureClose ?? 'world_map',
  };
}

/**
 * 프롬프트에 넣을 월드 상태 참조 (공통규칙 §5).
 *
 * 이 스테이지가 선언한 키 중 현재 충족된 것만 고른다. 전체 월드 상태를 통째로
 * 보내지 않는다 — 프롬프트 4000토큰 안에서 판정 기준표가 밀려나지 않게 하기 위해서다.
 * 선언되지 않은 상태가 충족되어 있어도 NPC는 모르는 것으로 쓴다.
 */
export function activeWorldStateReferences(
  stage: StageDefinition,
  worldStateKeys: string[],
): Record<string, string> {
  const declared = stage.worldStateReferences ?? {};
