 ...
 * 치명적 행동 신호.
 *
 * 공통규칙 §5의 원안은 이 값을 evaluation 객체 안에 두었으나 밖으로 분리했다.
 * §2 종료 순서에서 치명적 행동은 confirm·revoke 병합보다 **먼저** 검사되고,
 * 걸리면 합의를 병합하지 않고 끝낸다 — 문서가 이미 둘을 다른 단계로 두고 있다.
 * 그래서 evaluation은 "합의 키 → 이벤트" 맵으로만 두고 이 값은 별도 단계로
 * 처리한다. 판정 결과는 문서와 동일하다.
 * (부수적으로, 합의 키 이름과 충돌할 여지도 사라진다.)
 *
 * 이 신호만은 서버가 재검증할 수단이 없어 사실상 LLM 판단이 그대로 결과가 된다.
 * 합의 키는 "LLM이 번역 → 서버가 값을 대조"하는 이중 확인이 걸리지만,
 * "명시적 협박인가"는 구조체 비교로 확인할 수 없다.
 * "LLM은 성공·실패를 결정하지 않는다"(§5)는 원칙의 유일한 예외다.
 */
